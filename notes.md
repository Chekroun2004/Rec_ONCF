...
| **Rapport + 26 figures** | ✅ | `rapport_pfa_v2.tex` à jour (figure `app_mob.png` ajoutée au §1.1.2), layouts ONCF. Section **« Validation du Mécanisme de Réentraînement »** ajoutée (baseline + jour 1, tableaux, pas de figure). ⚠️ `rapport_pfa_v2.tex` est **gitignoré** (« not published to repo ») → sync manuel vers Overleaf |
| **Rapport — section Interface + tests réécrits (2026-05-23)** | ✅ | Nouvelle **section « Interface de Démonstration »** (Sprint 2) avec figure **`capp.png`** (vraie capture d'écran de l'interface, fournie à la main — le mockup matplotlib `interface_demo.png` a été retiré). Table endpoints corrigée (`GET /`, `GET /models`, variants a..d). Suite de tests réécrite **17 fichiers → 6 fichiers thématiques**. Section A/B Testing recentrée sur la **stabilité du pipeline**. Décompte figures **24 → 25**. |
| **Rapport — bug compilation corrigé (2026-05-23)** | ✅ | `\midrulees` → `\midrule` (ligne ~1955, table « Réentraînement Quotidien ») — typo préexistant qui bloquait la compilation. Vérifié : PDF produit. |
...
| **Rapport — anti-débordement global (2026-05-24)** | ✅ | **0 `Overfull \hbox`** sur 82 pages (vérifié en compile diagnostique locale). Préambule : `hyphenat[htt]` + `seqsplit` + `emergencystretch 3em` + `tolerance 2000` (césure des `\texttt` longs), `xurl` (URLs biblio). Tables converties : baselines + cold-start en `tabularx{\textwidth}` à colonne `X`, segments en `\small`. Abréviations + hardware en `\small` + `\dimexpr-20pt` (non-convergence tabularx résiduelle). Logos couverture bornés `width=\linewidth`. **Débordement vertical** (Table 5.3 « Couverture des tests » recouvrait le n° de page) : convertie en `xltabular` (`\usepackage{xltabular}`) → se coupe entre pages, en-tête répété + pied « suite page suivante ». |
| **Rapport — réduction de longueur (2026-05-25)** | ✅ | **87 → 75 pages** (compile vérifiée). **Niveau 1** : Annexe « Schéma de l'API » + Annexe « Extrait du code » supprimées (capture `code_recommend.png` déplacée dans le corps §2.3) ; Annexe « Guide de Déploiement » condensée (9→2 p : gardé pré-requis + quickstart Docker, retiré venv/observabilité/retrain/troubleshooting/rollback). Annexes restantes : A Métriques, B Commandes, C Guide. **Niveau 2** : §1.4 (3 réf. européennes → tableau), §2.5 (3 sous-sections schémas fusionnées), §6.5.4 (sous-sous-section « Divergence » retirée). **Niveau 3** : 6 figures retirées (CI/CD, bilan nettoyage, encodage cyclique, archi API, latence FastPreproc, Task Scheduler) → **24 figures** (UML tous conservés sur demande). **⚠️ Annexes A/B/C toutes supprimées depuis — voir « Rapport — finalisation (2026-05-25) ».** |
| **Rapport — listings code → captures réelles (2026-05-24)** | ✅ captures fournies | Les **5 listings Python** (`lstlisting`) remplacés par macro `\codeimg{fichier.png}{légende}` (repli `\imgplaceholder` auto via `\IfFileExists` → le rapport compile sans les PNG). Captures VS Code à fournir dans `pic/` : `code_xgb_hyperparams.png` (training.py 32–83), `code_compute_inference_row.png` (features.py 159–205), `code_lifespan.png` (main.py 49–90), `code_fastpreprocessor.png` (training.py 229–268), `code_recommend.png` (recommender.py `_recommend_core` 133–187). Listings **bash/JSON gardés en LaTeX** (commandes/payloads, pas du code source). **Les 5 captures `code_*.png` sont fournies dans `pic/` (2026-05-25).** |
| **Rapport — finalisation (2026-05-25)** | ✅ | **Annexes A/B/C toutes supprimées** ; métriques détaillées relocalisées dans le corps (nouvelle sous-section « Métriques Offline Détaillées du Modèle de Production », fin chapitre Phase 3 — tableau `tab:offline-detail` : HR@1 0.7691 / HR@3 0.9100 / MRR@3 0.8333 + 393 344 train / 98 261 test / 1 011 classes + empreinte `4d0dfd12e0b60341`). Corrections : ligne « Autres 4,4 ms / 19,1 % » au tableau latence (colonne « Part » somme à 100 %), phrase distinguant micro-bench vs p50 API, « pré-Covid » → « les plus anciennes », commentaire `scripts (01 a 12)`, mention « sur CPU » retirée. Conflit temps d'entraînement réglé (63,6 min supprimé avec l'annexe → seul ~43 min subsiste). **Relecture intégrale** : bug `\img[0.99]`→`[0.99\textwidth]` (fig. code_recommend), coquilles (« nôtre », « réentraînement », « candidate generator »), glossaire nettoyé (OE/OHE/CPU/TDD retirés), **bibliographie = 14 réf. toutes citées inline** (après aller-retour court→restaurée), `app_mob.png` ajoutée (**25 figures**). Fichier **2357 lignes**. ⚠️ **Non recompilé (Overleaf only)** → resync + recompiler. |
...
> **Rapport (2026-05-26) — 26 figures.** 164/164 tests. Les **26 figures** + 5 captures de code (`code_*.png`) + `capp.png` + `capp_res.png` + `app_mob.png` sont attendues dans `pic/`. **`capp_res.png` (panneau résultats) à capturer manuellement** (lancer l'API, faire une requête, screenshot de la zone résultats → sauver dans `pic/capp_res.png`). Puis **resynchroniser `rapport_pfa_v2.tex` et `pic/capp_res.png` vers Overleaf et recompiler**.

#### 1. Dernier pas — Overleaf
- **Copier `rapport_pfa_v2.tex` (local, gitignoré) → Overleaf, puis recompiler.** Vérifier : nouveau tableau `tab:offline-detail` (Phase 3) présent dans la « Liste des Tableaux », **0 `Overfull \hbox`**, pagination (~6 p de moins après suppression des annexes).
- S'assurer que `pic/` est complet sur Overleaf : 26 figures dont `oncf.png`, `LogoFsr.png`, `app_mob.png`, `pytest_output.png`, `capp.png`, **`capp_res.png`** et les 5 `code_*.png`.

#### 2. Corrections appliquées le 2026-05-25 (session feedback rapport)
- **Annexes A/B/C supprimées** ; sous-section « Métriques Offline Détaillées du Modèle de Production » ajoutée en fin de chapitre Phase 3 (`tab:offline-detail` : HR@1 0.7691 / HR@3 0.9100 / MRR@3 0.8333 + 393 344 train / 98 261 test / 1 011 classes + empreinte `4d0dfd12e0b60341`).
- Tableau latence : ligne « Autres 4,4 ms / 19,1 % » → la colonne « Part » somme à 100 %.
- Phrase ajoutée distinguant le micro-benchmark (calcul seul, hors HTTP) des latences p50/p99 API (round-trip FastAPI complet).
- « pré-Covid (2018–2020) » → « les plus anciennes (2018–2020) ».
- **Conflit temps d'entraînement réglé** : 63,6 min (ex-annexe A) supprimé → seul **~43 min** (Perspectives) subsiste, cohérent.
- Mineurs : commentaire `scripts (01 a 12)`, mention « sur CPU » retirée.
